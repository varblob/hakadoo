...
jQuery.hakadoo = ( function($) {
    ...
    var _questions = [{
	      ...
	      example: 'run(5) -> 15',
	      answer_key: 'function(n) { if (isNaN(n)) { return false; } return n * (n + 1)/2 }',
	      alwaysTest: [0, 1, 10],
	      ...
	      answer_key: 'function(s) { return s.split(" ").reverse().join(" "); }',
	      ...
	      answer_key: 'function(s) { var result = 0; for (var i = 0; i < s.length; i++) { if (s[i] == 13) { i++; } else { result += s[i]; } } return result; }',
    ...
    function _validate(questionId, userCode){
      var question = _questions[questionId]
				...
				, attempt = $.globalEval(question.answer_key);      
